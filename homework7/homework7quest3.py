...
# уменьшение, умножение и обычное (не целочисленное) деление клеток, соответственно. В методе деления должно осуществляться о
# кругление значения до целого числа.
...
class Cell:
    ...
    def __sub__(self, other):
        residual = self.amount - other.amount
        if residual > 0:
            return residual
        else:
            return f'Результат вычитание клеток меньше ноля'
    ...
    def make_order(self, nucleus):
        a = [('*'* nucleus) for el in range(int(self.amount//nucleus))]
        a.append('*' * (self.amount % nucleus))
        b = ['\n'.join(a)]
